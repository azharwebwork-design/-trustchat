    ...
    <div className="mx-auto max-w-[1200px]">
      ...
      <Card className="premium-dark relative overflow-hidden p-7 text-white md:p-9">
        ...
        <div className="relative flex flex-col gap-7 md:flex-row md:items-center">
          ...
          <div className="flex gap-3"><ComingSoonButton className="flex items-center gap-2 rounded-xl border border-white/20 px-5 py-3 text-sm font-bold">إلغاء الاشتراك</ComingSoonButton><ComingSoonButton className="flex items-center gap-2 rounded-xl bg-brand px-5 py-3 text-sm font-bold">ترقية الخطة</ComingSoonButton></div>
        </div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <Card className="p-6"><div className="flex items-center justify-between"><div><h2 className="font-bold">وسيلة الدفع</h2><p className="muted mt-1 text-xs">البطاقة المستخدمة للتجديد التلقائي</p></div><CreditCard className="text-brand" size={21}/></div><div className="mt-6 flex items-center gap-4 rounded-2xl border border-ui p-4"><span className="grid h-10 w-16 place-items-center rounded-lg bg-blue-600 text-xs font-black italic text-white">VISA</span><div className="flex-1"><b className="text-sm" dir="ltr">•••• •••• •••• 4242</b><p className="muted mt-1 text-[10px]">تنتهي في 09/28</p></div><span className="flex items-center gap-1 text-[10px] text-brand-dark dark:text-brand"><Check size={13}/> أساسية</span></div><ComingSoonButton className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border border-ui py-2.5 text-xs font-bold">تحديث وسيلة الدفع</ComingSoonButton></Card>
        <Card className="p-6"><div className="flex items-center justify-between"><div><h2 className="font-bold">بيانات الفوترة</h2><p className="muted mt-1 text-xs">تظهر هذه البيانات في فواتيرك</p></div><FileText className="text-brand" size={21}/></div><div className="mt-6 space-y-3 text-xs"><p><span className="muted inline-block w-28">اسم المنشأة</span> شركة TrustChat للتقنية</p><p><span className="muted inline-block w-28">الرقم الضريبي</span> 310123456700003</p><p><span className="muted inline-block w-28">البريد</span> billing@company.sa</p></div><ComingSoonButton className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl border border-ui py-2.5 text-xs font-bold">تعديل البيانات</ComingSoonButton></Card>
      </div>
      <Card className="mt-6 overflow-hidden"><div className="border-b border-ui p-5"><h2 className="font-bold">سجل الفواتير</h2><p className="muted mt-1 text-xs">حمّل فواتير اشتراكك السابقة</p></div><div className="divide-y divide-[color:var(--border)]">{invoices.map(([number,date,total,status])=><div key={number} className="grid grid-cols-[1.3fr_1fr_1fr_1fr] items-center px-5 py-4 text-xs"><b>{number}</b><span className="muted">{date}</span><span>{total}</span><div className="flex items-center justify-between"><span className="rounded-full bg-brand/10 px-3 py-1 text-brand-dark dark:text-brand">{status}</span><button disabled title="تنزيل الفاتورة قريبًا" aria-label="تنزيل الفاتورة قريبًا" className="muted cursor-not-allowed opacity-45"><Download size={16}/></button></div></div>)}</div></Card>
    </div>
  );
}
