...
export default async function Dashboard(){
  ...
  return <div className="mx-auto max-w-[1500px]">
    <PageTitle title={`مرحباً، ${user.name.split(" ")[0]}`} description="إليك ما يحدث في مساحة عملك اليوم." action={<button className="flex items-center justify-center gap-2 rounded-xl bg-brand px-5 py-3 text-sm font-bold text-white"><Plus size={18}/>محادثة جديدة</button>}/>
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[1.45fr_.8fr]">
      <Card className="p-5 md:p-6">
        <div className="flex items-center justify-between"><div><h2 className="font-bold">نشاط المحادثات</h2><p className="muted mt-1 text-xs">آخر 7 أيام</p></div><select className="rounded-lg border border-ui bg-transparent px-3 py-2 text-xs"><option>هذا الأسبوع</option></select></div>
    ...
    <Card className="mt-5 overflow-hidden"><div className="flex items-center justify-between border-b border-ui p-5"><div><h2 className="font-bold">أحدث المحادثات</h2><p className="muted mt-1 text-xs">آخر تفاعلات عملائك</p></div><button className="text-xs font-bold text-brand-dark dark:text-brand">عرض الكل</button></div><div className="divide-y divide-[color:var(--border)]">{conversations.map(x=><div key={x.id} className="flex items-center gap-3 p-4 hover:bg-black/[.015] dark:hover:bg-white/[.02]"><Avatar initials={x.initials} color={x.color}/><div className="min-w-0 flex-1"><b className="block text-xs">{x.name}</b><p className="muted truncate text-xs">{x.msg}</p></div><span className="muted text-[10px]">{x.time}</span>{x.unread>0&&<span className="grid size-5 place-items-center rounded-full bg-brand text-[9px] text-white">{x.unread}</span>}</div>)}</div></Card>
  </div>
}
