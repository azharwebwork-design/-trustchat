...
export default function ChatbotPage() {
  return (
    <div className="mx-auto max-w-[1500px]">
      <PageTitle title="منشئ الرد الآلي" description="صمّم تجربة محادثة ذكية بالسحب والإفلات." action={<div className="flex gap-2"><button className="flex items-center gap-2 rounded-xl border border-ui surface px-4 py-2.5 text-sm font-bold"><Play size={17} /> اختبار</button><button className="flex items-center gap-2 rounded-xl bg-brand px-4 py-2.5 text-sm font-bold text-white"><Save size={17} /> حفظ ونشر</button></div>} />
      <Card className="grid min-h-[690px] overflow-hidden lg:grid-cols-[240px_1fr_270px]">
        <aside className="border-l border-ui p-4">
          ...
          <div className="mt-8 rounded-2xl bg-brand/10 p-4"><Sparkles size={20} className="text-brand" /><b className="mt-3 block text-xs">اقتراح بالذكاء الاصطناعي</b><p className="muted mt-2 text-[10px] leading-5">دع TrustChat ينشئ مساراً أولياً بناءً على هدفك.</p><button className="mt-3 text-[10px] font-bold text-brand-dark dark:text-brand">إنشاء مسار</button></div>
        </aside>
        <section className="relative min-h-[620px] overflow-auto bg-[radial-gradient(circle,var(--border)_1px,transparent_1px)] [background-size:24px_24px]">
          ...
          <button className="absolute right-[27.5%] top-[535px] grid size-9 place-items-center rounded-full border-2 border-dashed border-brand bg-[color:var(--card)] text-brand"><Plus size={17} /></button>
        </section>
        <aside className="hidden border-r border-ui p-5 lg:block">
          ...
          <label className="mt-6 block text-xs font-bold">اسم العنصر<input defaultValue="رسالة ترحيبية" className="mt-2 w-full rounded-xl border border-ui bg-transparent px-3 py-2.5 outline-none focus:border-brand" /></label>
          <label className="mt-5 block text-xs font-bold">نص الرسالة<textarea defaultValue="مرحباً بك في متجرنا! كيف يمكننا مساعدتك اليوم؟" rows={5} className="mt-2 w-full resize-none rounded-xl border border-ui bg-transparent p-3 leading-6 outline-none focus:border-brand" /></label>
          <label className="mt-5 block text-xs font-bold">وقت الانتظار<select className="mt-2 w-full rounded-xl border border-ui bg-transparent px-3 py-2.5"><option>إرسال فوراً</option><option>بعد ثانية</option></select></label>
...
function NodeTool({ icon: Icon, title, color }: { icon: React.ElementType; title: string; color: string }) {
  return <div className="flex cursor-grab items-center gap-3 rounded-xl border border-ui p-3"><span className={`grid size-9 place-items-center rounded-lg ${color}`}><Icon size={18} /></span><b className="text-xs">{title}</b></div>;
}
